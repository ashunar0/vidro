...
- [0017-ssr-phase-b-router-server-mode](./0017-ssr-phase-b-router-server-mode.md) — SSR Phase B Step B-2b: Router に `ssr` prop + `preloadRouteComponents` helper、ErrorBoundary の `isServer` 分岐。`renderToString(<Router ssr={...} />)` で sync fold (RSC-like 時に `ServerRouter` / `ClientRouter` 分離予定)
- [0018-ssr-phase-b-create-server-handler-integration](./0018-ssr-phase-b-create-server-handler-integration.md) — SSR Phase B Step B-2c: `createServerHandler` の navigation 分岐に renderToString を統合 + production manifest を全 entry 実 import 化 + `mount(fn, target)` を fresh render (target.replaceChildren) として確定 (Step B-3 で `hydrate` と対比立てる)
- [0019-ssr-phase-b-hydrate-primitive](./0019-ssr-phase-b-hydrate-primitive.md) — SSR Phase B Step B-3a: `hydrate(fn, target)` + JSX transform 強化 (JSXText/子位置 expr を `_$text`/`_$dynamicChild` で h() より先に解決) + HydrationRenderer (post-order cursor)。ErrorBoundary / Show / Switch / For / Router は B-3b 以降
- [0067-per-route-types-codegen](./0067-per-route-types-codegen.md) — Per-route `+types` codegen で `Route` namespace 生成 (`PageProps` / `LayoutProps` / `LoaderArgs` / `ActionArgs` / `ErrorBoundaryProps`)、`./+types` per-directory import + tsconfig `rootDirs`、ADR 0011 中央 RouteMap を per-route で拡張、型貫通 #1 (URL pattern → params 自動推論) 着地
- [0068-action-placement-and-resource-route](./0068-action-placement-and-resource-route.md) — `index.server.tsx` の `action` export 認識 + `server.ts` 単独 directory も resource route として認識 (form dogfood で発見した co-location 違反 + resource route 機構サポート外を解決)
- [0069-form-control-primitive](./0069-form-control-primitive.md) — `@vidro/form` opt-in pack で `formControl({schema})` primitive (= signal-based state machine + bind/field/error/pending/setFieldErrors/reset)、型貫通 #4 完成、AppRouter mode の island form 主軸
- [0070-server-function-pattern](./0070-server-function-pattern.md) — Server function pattern (`serverFn()` wrapper + `server.ts` / `*.server.tsx` named export + 場所自由化 + mode 排他 + 自動 fetch stub + URL = src 相対 path (routes/ 内のみ strip) + self-managed middleware default + routes/ 配置 bonus + Hono `c` subset)、Server Actions 4 脆弱性を構造的に削る、3-trigger Path 1 完成、型貫通 #4+#9 着地
- [0071-vidro-zod-opt-in-pack](./0071-vidro-zod-opt-in-pack.md) — `@vidro/zod` opt-in pack で `validator(schema)` middleware + `fieldsFromZodError` helper、ADR 0070 の schema 重複問題着地、Hono `@hono/zod-validator` 直系
- [0072-server-fn-pure-handler-signature](./0072-server-fn-pure-handler-signature.md) — serverFn handler signature を pure service form (`(input, c?: Context) => R` 末尾 optional) に改修、ADR 0070 論点 6+7 部分 supersede、9 割 handler が pure に書ける + 1 割 edge case で c に手が届く
- [0073-server-fn-object-slot-signature](./0073-server-fn-object-slot-signature.md) — serverFn signature を object slot form に改修 (`serverFn({ middleware, validator, handler: ({ params, data, ctx }) => R })`)、ADR 0070 + 0072 + 0071 supersede、F1 (validator × URL [slug] 共存不可) 構造的解決、流派 1 (object 1 個) + 型付き ctx + c 完全排除 + AI フレンドリー優先
