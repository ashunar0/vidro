...
function renderServerEntry(): string {
  ...
  //
  // Cloudflare Workers Assets (SPA mode) との組み合わせ:
  //   1. `/__loader` (loader endpoint) → createServerHandler が JSON で応答
  //   2. それ以外 → `env.ASSETS.fetch(request)` に委譲し、static hit か
  //      `not_found_handling: "single-page-application"` 経由で index.html に
  //      fallback させる (client Router が初期化されて最終的な render を行う)
  //
  // SSR を後で足す時は 2 の分岐が `env.ASSETS.fetch` から `renderHTML(request)`
  // に差し替わるだけで、構造は共通。
  const lines: string[] = [];
  lines.push("// AUTO-GENERATED by @vidro/plugin routeTypes() — DO NOT EDIT");
  ...
  lines.push("");
  lines.push("const loaderHandler = createServerHandler(routeManifest);");
  lines.push("");
  lines.push("export default {");
  lines.push("  async fetch(request: Request, env: Env): Promise<Response> {");
  lines.push("    const url = new URL(request.url);");
  lines.push('    if (url.pathname.startsWith("/__loader")) {');
  lines.push("      return loaderHandler(request);");
  lines.push("    }");
  lines.push("    // SPA fallback: assets に投げれば hit すれば static、miss は");
  lines.push("    // not_found_handling で index.html に fallback する (wrangler 側設定)。");
