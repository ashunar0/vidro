...
import { serverComponent, type ServerComponentOptions } from "./server-component";
...
export type VidroOptions = {
  /**
   * `@vidro/router` を使う場合 true。routeTypes + serverBoundary + serverComponent
   * が plugin chain に追加される。advanced 設定が要る場合は object を渡す。
   */
  router?: boolean | (RouteTypesOptions & ServerBoundaryOptions & ServerComponentOptions);
};
...
export function vidro(options: VidroOptions = {}): Plugin[] {
  ...
  if (options.router) {
    ...
    // ADR 0060: .server.tsx を stub virtual module に置き換えて island だけ client
    // bundle に残す。serverBoundary より後に並ぶが enforce: "pre" 同士で順序依存
    // しない (= load hook の応答が早く返った方が勝つ、両者の id 判定は disjoint)。
    plugins.push(serverComponent());
  }
